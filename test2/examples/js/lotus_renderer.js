...
function growth_percent_calc(confirmed,old_confirmed) {
	...
	growth_calc = growth_calc.toFixed(2);
...
function getFormattedDate(date) {
  ...
  if (this.readyState == 4 && this.status == 200) {
	...
	for (var i = 0; i < growth_calc.length; i++) {
	    ...
	    var old = county.old;
	    var fips = county.fips;
	    var county_growthrate = growth_percent_calc(today,old);
	...
	for (var i = 0; i < growthrates.length; i++) {
	    ...
	    if(county.growthrate >= 30) {
	  	growthtrendhighest.push(county.fips);   
	    }
	    else if(county.growthrate < 30 && county.growthrate >= 20) {
                growthtrendhigh.push(county.fips);  
            }
	    else if(county.growthrate < 20 && county.growthrate >= 0) {
...
getData(geometricLinks);
  ...
  function toggleLinks2 (linkobject, id) {
    if (linkobject.visible == true) {
      ...
      linkobject.visible = true
      document.getElementById(id).style.backgroundColor = '#32dcfe'
    }
  }
